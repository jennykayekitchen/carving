...
const createWoodBlock = (woodProperty) => {
    // Return an object with 4 properties.
    const woodObject = {
        type: "wood block",
        length: 10,
        material: "pine",
        purposeType: woodProperty,
        purpose: "flute"
    }
    return woodObject
};
...
const createBeautifulCarving = (woodObject) => {
    ...
    const carvingStringRep = `The ${woodObject.length}-inch, ${woodObject.material} ${woodObject.type} was carved into a ${woodObject.purposeType} ${woodObject.purpose}.`
    
    return carvingStringRep
};

// The function returns a value, so that value is stored in a variable
const woodBlock = createWoodBlock("wooden");
